...
# Filtro por Actividad (nuevo)
actividades = ["Todas"] + sorted(cursos[cursos['Ingresante'] == "SI"]['Actividad'].unique().tolist())
...
# ----------------------
# SEGUNDA BARRA (FILTROS ORIGINALES: PERIODO Y PUESTO)
# ----------------------
col1, col2 = st.columns(2)

with col1:
    periodos = ["Todos"] + sorted(cursos[cursos['Ingresante'] == "SI"]['Periodo Valoración'].unique().tolist())
...
with col2:
    puestos = ["Todos"] + sorted(cursos[cursos['Ingresante'] == "SI"]['Puesto Tipo'].unique().tolist())
...
# ----------------------
# TERCERA BARRA (DENTRO DEL EXPANDER - OPCIONAL)
# ----------------------
...
with st.expander("📋 LISTADO DE POSTULANTES", expanded=True):
    ...
    # Filtramos los valores que son numéricos antes de convertirlos a datetime
    for col in ['Fecha Inicio', 'Fecha Fin']:
        if col in df_filtrado.columns:
            # Asegurarnos de que solo tratemos con valores numéricos
            df_filtrado[col] = pd.to_numeric(df_filtrado[col], errors='coerce')
            
            # Convertir solo las celdas numéricas a datetime
            df_filtrado[col] = pd.to_datetime(df_filtrado[col], errors='coerce', origin='1899-12-30', unit='D')
    
            # Ahora formateamos las fechas en el formato deseado (DD/MM/YYYY)
